...
namespace MG
{
	...
	void CommandHandler::HandleNewCommand(CommandLineArguments* args, int index)
	{
		...
		else
		{
			...
			Application::Print("Project Wizard", "What would you like to name your new C++ project?");
	...
	void CommandHandler::HandleGenerateCommand()
	{
		...
		{
			Application::Print("Magnet", "In order to generate, run this command at the root of your project, where .magnet can be found.");
		...
		std::string generateCommand = "cmake -S . -B " + projectName + "/Build -G Xcode -DCMAKE_BUILD_TYPE=Debug";
		
		int status = std::system(generateCommand.c_str());
		if(status != 0)
		{
			Application::Print("Magnet", "CMake failed to generate project files. See messages above for more information.");
			
			return;
		}
				
		Application::Print("Magnet", "Successfully generated project files. Run `magnet build` next.");
	}
	...
	void CommandHandler::HandleBuildCommand()
	{
		...
		if(status != 0)
		{
			Application::Print("Magnet", "CMake couldn't build the project. See messages above for more information. Have you tried generating your project files first? If not, run `magnet generate`.");
			
			return;
		}
		
		Application::Print("Magnet", "Successfully built project. Run `magnet go` to launch your app.");
	}
	...
	void CommandHandler::CreateNewProject(const std::string& name, const std::string& type)
	{
		...
		Application::Print("Project Wizard", name + " has been created.\nNext steps: `cd "
						   + name + " && magnet generate` to generate project files.");
	}
